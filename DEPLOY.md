...
The production GitHub sync is implemented as a chain of bounded Workflow instances under the Workers Free 50-subrequest ceiling. Repository/PR collection handles one PR per instance, reaction collection handles one validation comment per instance, and duplicate closing handles one PR (two GitHub writes) per instance. A D1 lease prevents overlapping canonical pipelines. The Workflow pauses before its tracked daily step allowance is exhausted and resumes after the UTC reset.
